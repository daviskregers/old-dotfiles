" Taken and customized from https://raw.githubusercontent.com/vim-test/vim-test/master/autoload/test/php/phpunit.vim
...
function! test#php#customphpunit#build_args(args, color) abort
  ...
  if a:color
    let args = args + ['--colors'] + ['--enforce-time-limit'] + ['--fail-on-warning'] + ['--stop-on-error']
  endif
